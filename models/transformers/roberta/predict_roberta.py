...
def main():
    ...
    MODEL_DIR = Path(__file__).resolve().parent / "roberta_sentiment"
    ...
    if MODEL_DIR.exists():
        print("Contents:", [p.name for p in MODEL_DIR.iterdir()])
    ...
    with out_path.open("w", encoding="utf-8") as f:
        for i in tqdm(range(0, len(lines), bs), desc="Predict"):
            batch = lines[i:i+bs]
    ...
    print("Wrote:", out_path, "Total:", len(lines))
